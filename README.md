...
> Curated techniques for reducing LLM token usage
...
|---------|-------------|--------|
| Core — Curated foundation for tokens workflows | Core foundation — the primary abstraction this repo builds on | [docs](#) |
| Execution — task decomposition and tokens routing | Execution layer — how tasks get decomposed and routed | [docs](#) |
| Integration — Claude Code + optimization connectivity | Integration layer — connecting to external tools and APIs | [docs](#) |
...
```bash
# Quick start
python3 main.py --task "tokens task here"

# Power user shortcut
python3 main.py --tokens --fast
...
Use Awesome-Collection-Token-Reduction-main to automate curated techniques for reducing llm token usage. Perfect for agencies, freelancers, and AI-first teams running tokens workflows at scale.
...
```bash
# Add to CLAUDE.md
echo "Auto-activate: tokens, optimization" >> ~/.claude/CLAUDE.md

# Or load skill directly
/load-skill Awesome-Collection-Token-Reduction-main
```
...
{
  "hooks": {
    "PostToolUse": [{"matcher": "tokens", "command": "python3 ~/repos/Awesome-Collection-Token-Reduction-main/main.py"}]
  }
}
```
...
```bash
# Fork and clone
gh repo fork hmzainjamil/Awesome-Collection-Token-Reduction-main --clone
cd Awesome-Collection-Token-Reduction-main
